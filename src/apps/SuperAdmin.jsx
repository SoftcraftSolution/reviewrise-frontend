...
export default function SuperAdmin({ user, onLogout }) {
  ...
  const [modal,    setModal]  = useState(null)   // 'brand'|'ad'|'banner'
  ...
  const handleAddBrand = async (formData) => {
    ...
    setModal(null)
    setNewCreds(r.owner_credentials)
    load()
  }
  ...
  return (
    <div className="dash-layout fade" style={{position:'relative'}}>
      ...
      <div className="main-area">
        <div className="topbar">
          <div className="topbar-l">
            <div className="t1">{{brands:'Brand Management',overview:'Platform Overview',customers:'Customer Intelligence',ads:'Ads & Banners',qr:'QR Generator'}[tab]}</div>
        ...
        <div className="content-area">
          {err && <div style={{background:'var(--redbg)',borderRadius:10,padding:'10px 14px',fontSize:12,color:'var(--red)',marginBottom:16,display:'flex',gap:10,alignItems:'center'}}>⚠ {err} <button className="btn sm" onClick={load}>Retry</button></div>}
            ...
            <div className="fade">
              {brands.length === 0 ? (
                <div style={{textAlign:'center',padding:'80px 20px'}}>
                  ...
                  <div style={{color:'var(--muted)',fontSize:13,marginBottom:24,lineHeight:1.7}}>Add your first real client. Their login will be auto-generated.</div>
      ...
      {/* ── MODALS ── all use fixed positioning ── */}

      {modal==='brand' && (
        <Modal onClose={()=>setModal(null)} maxWidth={540}>
          <AddBrandForm onClose={()=>setModal(null)} onAdd={handleAddBrand}/>
        </Modal>
      )}

      {newCreds && (
        <Modal onClose={()=>setNewCreds(null)} maxWidth={400}>
          <div style={{textAlign:'center',marginBottom:20}}>
            <div style={{fontSize:44,marginBottom:8}}>🎉</div>
            <div style={{fontSize:18,fontWeight:900,marginBottom:4}}>Brand Created!</div>
            <div style={{fontSize:13,color:'var(--muted)'}}>Share these with the brand owner</div>
          </div>
          <div style={{background:'#161D28',border:'2px dashed rgba(233,184,74,.3)',borderRadius:14,padding:20,marginBottom:20}}>
            {[['Login URL','https://reviewrise-frontend.vercel.app/brand','var(--blue)'],['Email',newCreds.email,'var(--gold)'],['Password',newCreds.password,'var(--green)']].map(([l,v,c])=>(
              <div key={l} style={{marginBottom:12}}>
                <div style={{fontSize:10,color:'var(--muted)',textTransform:'uppercase',letterSpacing:'.08em',marginBottom:3,fontWeight:700}}>{l}</div>
                <div style={{fontFamily:'monospace',fontSize:14,fontWeight:700,color:c,wordBreak:'break-all'}}>{v}</div>
              </div>
            ))}
          </div>
          <button className="btn primary" style={{width:'100%',justifyContent:'center',fontSize:14}} onClick={()=>{
            navigator.clipboard?.writeText(`URL: https://reviewrise-frontend.vercel.app/brand\nEmail: ${newCreds.email}\nPassword: ${newCreds.password}`)
            setNewCreds(null)
          }}>📋 Copy & Close</button>
        </Modal>
      )}
      ...
      {modal==='ad' && (
        <Modal onClose={()=>setModal(null)}>
          <div style={{fontSize:16,fontWeight:900,marginBottom:4}}>Create Ad Campaign</div>
          <div style={{fontSize:12,color:'var(--muted)',marginBottom:16}}>Watch 3 ads = customer earns a coupon</div>
      ...
      {modal==='banner' && (
        <Modal onClose={()=>setModal(null)}>
          ...
          <div style={{fontSize:12,color:'var(--muted)',marginBottom:16}}>Shown on Discovery App homepage</div>
          ...
          {confirm==='RESET_ALL' ? (
            <>
              <div style={{textAlign:'center',marginBottom:20}}>
                ...
                <div style={{fontSize:13,color:'var(--muted2)',lineHeight:1.7}}>
                  Permanently removes all brands, reviews, coupons & dummy data.<br/>
            ...
            <>
              ...
              <div style={{fontSize:13,color:'var(--muted2)',lineHeight:1.7,marginBottom:20}}>
                Permanently removes this brand and all its data. Cannot be undone.
              </div>
              <div style={{display:'flex',gap:10}}>
...
function QRGenerator({ brands, onCreated }) {
  ...
  if (!brands.length) return <div style={{textAlign:'center',padding:60,color:'var(--muted)'}}>Add brands first, then generate QR codes.</div>

  return (
    <div className="fade" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,alignItems:'start'}}>
      <div className="card-box">
        <div style={{fontWeight:800,fontSize:14,marginBottom:4}}>📲 QR Code Generator</div>
        <div style={{fontSize:12,color:'var(--muted)',marginBottom:20}}>Generates a real scannable QR for the review page</div>
        ...
        <div className="form-row">
          ...
          <input className="input" value={label} onChange={e=>{setLabel(e.target.value);setDone(false)}} placeholder="T1, Main Entrance, Counter..."/>
        </div>
        <div className="form-row">
        ...
        {brand?.google_place_id && (
          <div className="form-row">
            <label className="label">Place ID (for verification)</label>
        ...
        {done && <div style={{marginTop:12,padding:10,background:'var(--greenbg)',borderRadius:8,fontSize:12,color:'var(--green)'}}>✓ Saved! Download and print for {label}.</div>}
      </div>
      ...
      <div style={{background:'white',borderRadius:16,padding:24,display:'flex',flexDirection:'column',alignItems:'center',gap:10,boxShadow:'0 8px 32px rgba(0,0,0,.3)'}}>
        ...
        <img src={qrImg} alt="QR" width={180} height={180} style={{borderRadius:8,border:'2px solid #f0f0f0'}}/>
        ...
        <div style={{fontSize:9,color:'#bbb',textAlign:'center',maxWidth:180}}>Scan → Review on Google → Reward unlocked instantly</div>
      </div>
    </div>
  )
}
  ...
  return (
    <>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 10px'}}>
        <div className="form-row"><label className="label">Category</label><input className="input" value={f.category} onChange={e=>s('category')(e.target.value)} placeholder="Restaurant, Café, Salon..."/></div>
