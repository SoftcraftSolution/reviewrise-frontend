...
export default function SuperAdmin({ user, onLogout }) {
  ...
  const load = async () => {
    ...
    try {
      const [b,c,a,bn,s,fb] = await Promise.all([
      ...
      setCust(c); setAds(a); setBanners(bn); setStats(s); setFeedback(fb)
...
function AdForm({ onClose, onAdd, brands, initial, submitLabel='Create Ad' }) {
  ...
  return (
    <>
      ...
      <div className="form-row"><label className="label">Title</label>
        <input className="input" placeholder="e.g. Summer Special Offer" value={f.title} onChange={e=>setF(p=>({...p,title:e.target.value}))}/>
      </div>
      <div className="form-row"><label className="label">Description</label>
        <textarea className="input" style={{height:70,resize:'none'}} placeholder="Short promo text..." value={f.description} onChange={e=>setF(p=>({...p,description:e.target.value}))}/>
      </div>
      <div className="form-row"><label className="label">Image URL <span style={{color:'var(--muted)',fontWeight:400}}>(optional)</span></label>
        <input className="input" placeholder="https://..." value={f.image_url} onChange={e=>setF(p=>({...p,image_url:e.target.value}))}/>
        {f.image_url && <img src={f.image_url} alt="" style={{width:'100%',height:80,objectFit:'cover',borderRadius:8,marginTop:6}} onError={e=>e.target.style.display='none'}/>}
      </div>
      <div style={{display:'flex',gap:8}}>
        <button className="btn primary" style={{flex:1,justifyContent:'center'}} disabled={saving} onClick={async()=>{setSaving(true);try{await onAdd(f)}catch(e){alert(e.message)}setSaving(false)}}>
  ...
  return (
    <>
      ...
      <div className="form-row"><label className="label">Headline</label>
        <input className="input" placeholder="e.g. Get 20% OFF this weekend!" value={f.title} onChange={e=>setF(p=>({...p,title:e.target.value}))}/>
      </div>
      <div className="form-row"><label className="label">Banner Image URL <span style={{color:'var(--muted)',fontWeight:400}}>(optional — shows rich image card)</span></label>
        <input className="input" placeholder="https://..." value={f.image_url} onChange={e=>setF(p=>({...p,image_url:e.target.value}))}/>
        {f.image_url && <img src={f.image_url} alt="" style={{width:'100%',height:100,objectFit:'cover',borderRadius:8,marginTop:6}} onError={e=>e.target.style.display='none'}/>}
      </div>
      <div style={{display:'flex',gap:8}}>
        <button className="btn primary" style={{flex:1,justifyContent:'center'}} disabled={saving} onClick={async()=>{setSaving(true);try{await onAdd(f)}catch(e){alert(e.message)}setSaving(false)}}>
          {saving?'Saving...':submitLabel}
